...
access_key = "570634225-AnsM63tVCpI4yeFwpj6QfSJTwm3pUx6onf30fI2Z"
access_secret = "ykA5CW0lWpl3VDiIRqJ5rhJjsQc6fyt0pps22tLAywXUJ"
...
def collect_tweets_data(screen_name):
    ...
    tweet_in = api.user_timeline(screen_name=screen_name,tweet_mode="extended", include_rts = True, count=1000)
    ...
    # write the csv
    with open('tweeterScrapper2.csv', 'a', newline ='') as f:
        writer = csv.writer(f)    
...
if __name__ == '__main__':
    ...
    page_id = ["korantempo"]                 #screen_name
    flag = 0
    now = datetime.datetime.now()
    print("Data collected for time: ",now)
    if os.path.isfile('tweeterScrapper2.csv'):
        with open('tweeterScrapper2.csv', 'a',newline='') as f:
            writer = csv.writer(f)
            writer.writerow([" "," "])
        ...
        with open('tweeterScrapper2.csv', 'w',newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Account", "texts"])
        for i in page_id:
            collect_tweets_data(i)
            time.sleep(1)
        flag = 1
